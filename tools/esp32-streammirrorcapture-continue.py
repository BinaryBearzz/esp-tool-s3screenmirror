...
class ScreenStreamerApp:
    ...
    def create_widgets(self):
        ...
        # Checkbox: Continue if send fails
        self.continue_var = tk.BooleanVar(value=False)
        self.continue_check = ttk.Checkbutton(self.root, text="Continue if send fails", variable=self.continue_var)
        self.continue_check.grid(row=6, column=2, sticky='w', **pad)
        
        # Buttons
        self.refresh_button = ttk.Button(self.root, text="Refresh Screens", command=self.refresh_screens)
        self.refresh_button.grid(row=7, column=0, **pad)
        
        self.clear_button = ttk.Button(self.root, text="Clear Fields", command=self.clear_fields)
        self.clear_button.grid(row=7, column=1, **pad)
        
        self.stream_button = ttk.Button(self.root, text="Start Streaming", command=self.toggle_streaming)
        self.stream_button.grid(row=8, column=0, columnspan=3, sticky='we', **pad)
    ...
    def stream_loop(self):
        ...
        if not success:
            if self.continue_var.get():
                print("Warning: Failed to send frame, but continuing due to checkbox.")
            else:
